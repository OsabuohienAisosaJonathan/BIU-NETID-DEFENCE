...
class NetIDSFeatureNBased:
    ...
    def __init__(self, pretrained_model_path=None, preprocessed_dataset_location="datasets/processed.csv"):
        ...
        if pretrained_model_path and os.path.exists(pretrained_model_path):
            ...
            self.label_classifier = LogisticRegression(max_iter=1000, random_state=42)
            self.type_classifier = LogisticRegression(max_iter=1000, random_state=42)
    ...
    def load_datasets(self):
        ...
        if os.path.exists(self.preprocessed_dataset_location):
            ...
            netids = NetIDS()
            data = netids.load_datasets('../datasets') 
        return data
    ...
    def evaluate(self, df, feature_cols, label_col, type_col):
        ...
        print("[Attack Label Prediction Report]")
        print(classification_report(y_label_true, pred_label))

        attack_type_encoder = joblib.load("../models/attack_type_encoder.pkl")
        pred_type_int = attack_type_encoder.transform(pred_type) 
        print('pred_type_int:', pred_type_int)
        print("[Attack Type Prediction Report]")
        print(classification_report(y_type_true, pred_type_int))

    def save_model(self, path):
        """
        Save both the pretrained RF and the transfer classifiers.
        """
        joblib.dump({
    ...
    def load_model(self, path):
        """
        Load pretrained RF and transfer classifiers.
        """
